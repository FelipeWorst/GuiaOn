...
function inserirReserva(idReserva, dataReserva, dataVolta, pago) {
    if (encontrarReserva(dataReserva) >= 0) {
        let mensagem = "ja existe uma reserva agendada para o dia " + dataReserva
        return mensagem
    } else {
        let mensagem = {idReserva: id, dataReserva, dataVolta, pago };
        return men
        id++
    }
}
...
function deletarReserva(dataReserva) {
    ...
    if (indice !== -1) {
        ...
        let mensagem = "Reserva deletado com sucesso.";
        return mensagem
    } else {
        let mensagem = "O Reserva que você digitou não foi encontrado.";
        return mensagem
    }
}
